  ...
  const QuantityButton = () => {
    ...
    return (
      <div className="flex items-center w-full">
        <button onClick={handleDecrement} className="border-gray-600 font-semibold text-xs w-[30px] h-[30px] border ">-</button>
        <span className = "w-full border h-[30px] border-gray-600 text-center">{quantity}</span>
        <button onClick={handleIncrement} className="border-gray-600 font-semibold text-xs w-[30px] h-[30px] border ">+</button>
      </div>
    );
  };
...
const updatedCartItems = oldCartItems.map((oldCartitem) => {
  if (oldCartitem.name === product.name) {
    ...
    return { ...oldCartitem, quantity: oldCartitem.quantity + quantity }; // Update quantity
  }
...
console.log(JSON.parse(localStorage.getItem('cart')));
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center overflow-auto">
      <div className="bg-white w-full max-w-6xl mx-auto p-6 rounded-lg relative shadow-lg overflow-auto">
        <button onClick={onClose} className="absolute top-4 right-4 text-gray-500 hover:text-black text-3xl font-bold">
          &times;
        </button>
        ...
        <div className="flex flex-col lg:flex-row">
          ...
          <div className="w-full lg:w-1/2 p-6">
            ...
            {product.isMeasureable && (
              <button onClick={toggleSizingGuide} className="mb-4 text-black hover:text-gray md:underline">
                View Sizing Guide
              </button>
            )}
      ...
      {showSizingGuide && (
        <div className="fixed inset-0 bg-black bg-opacity-70 z-50 flex justify-end items-start">
          <div className="bg-white w-[550px] p-6 relative shadow-lg overflow-auto h-full">
            <button
              onClick={toggleSizingGuide}
              className="absolute top-4 left-4 text-gray-500 hover:text-black text-3xl font-bold"
            >
              &times;
            </button>
              ...
              <table className="min-w-full bg-white border-collapse border border-gray-300">
                <thead>
                  <tr>
                    ...
                    {sizes.map((size, index) => (
                      <th key={index} className="border border-gray-300 px-4 py-2 text-[15px] font-medium">
                        {size}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  ...
                  {[
                    ...
                    <tr key={index}>
                      <td className="border border-gray-300 px-4 py-2">{measurement.name}</td>
                      {measurement.values.map((value, idx) => (
                        <td key={idx} className="border border-gray-300 px-4 py-2">{value} cm</td>
                      ))}
                    </tr>
                  ))}
                </tbody>
